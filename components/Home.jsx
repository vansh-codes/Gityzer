...
function Home() {
  ...
  useEffect(() => {
    const mouseTrack = document.querySelector('.mouse-track');
    
    const createRipple = (e) => {
      // Ensure ripple does not happen on button or input elements
      if (e.target.closest('.search-button')) return;

      const ripple = document.createElement('div');
      ripple.classList.add('mouse-ripple');
      ripple.style.left = `${e.clientX - 75}px`; // Center ripple
      ripple.style.top = `${e.clientY - 75}px`;

      mouseTrack.appendChild(ripple);
    ...
    mouseTrack.addEventListener('mousemove', createRipple);

    return () => {
      mouseTrack.removeEventListener('mousemove', createRipple);
    };
  }, []);
    ...
    <>
      ...
      <div className="flex flex-col justify-between min-h-screen">
        ...
        <svg width="40px" height="40px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <h1 className="text-xl font-bold text-white glow-text">Gityzer</h1>
        </header>

        {/* Main Content */}
        <main className="flex flex-col justify-center items-center flex-grow mouse-track">
          <h1 className="text-4xl font-light tracking-widest mb-8 glow-text">
            Let's, Begin...
          </h1>
          <form className="flex items-center w-1/3 h-1/3" onSubmit={handleSubmit}>
            <div className="flex items-center bg-white rounded-full shadow-md p-2 w-full transition-all duration-300 ease-in-out transform hover:scale-105 hover:bg-blue-500 search-button">
              {/* GitHub Logo inside search bar */}
              <div className="px-3 github-icon">
                <svg
              ...
              <button
                type="submit"
                className="bg-purple-500 text-white p-3 rounded-full hover:bg-purple-600 focus:outline-none focus:ring-4 focus:ring-purple-500 focus:ring-opacity-50 transition duration-200 flex items-center justify-center search-button"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-5 w-5"
